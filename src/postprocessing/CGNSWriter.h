...
private:
	...
	std::map<std::string, int> _solutionNameToS;
protected:
...
public:
	//Constructor initialize required subsystems
	CGNSWriter(std::shared_ptr<ParallelManager>& MPIManager, std::shared_ptr<Logger> logger) : _MPIManager(MPIManager), _logger(logger) { };
	...
	void OpenFile(std::string fname) {		
		...
		if (ierror) {
			//Error while reading assuming no solutions
		...
		for (int S = 0; S<nSolutions; S++) {
			...
			if (gridLocation == GridLocation_t::CellCenter) {					
				_solutionNameToS[std::string(solName)] = S; //Add solution name
			};		
		};
	...
	void WriteGridToFile(std::shared_ptr<Grid>& grid) {
		...
		//_MPIManager->GatherCounts(grid.nCellsLocal, nCellsPerProcessor);						
	...
	void WriteSolution(Grid& grid, std::string solutionName) {		
		if (_MPIManager->IsMaster()) {	
			...
			_solutionNameToS[ solutionName ] = S;
		};
